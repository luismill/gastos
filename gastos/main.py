...
from settings import CATEGORY_DATABASE_ID
    ...
    try:
        ...
        if month and not record_exists(existing_records, fecha.strftime('%Y-%m-%d'), cuenta, gasto, ingreso):
            ...
            if response:
                ...
                logging.info(
                    f"Registro insertado: {nombre}, {fecha}, {cuenta}, gasto={gasto}, ingreso={ingreso}, subcategoria={subcategoria}"
                )
            else:
                logging.warning(
                    f"No se pudo insertar el registro: {nombre}, {fecha}, {cuenta}, gasto={gasto}, ingreso={ingreso}, subcategoria={subcategoria}"
                )
        else:
            if month:
                repeated_records[month] = repeated_records.get(month, 0) + 1
                logging.info(
                    f"Registro repetido (no insertado): {nombre}, {fecha}, {cuenta}, gasto={gasto}, ingreso={ingreso}"
                )
...
def export_notion_data(file_path: str, status_label=None) -> None:
    """Exporta registros de Notion a un CSV."""
    try:
...
def export_subcategorias(_: str, status_label=None) -> None:
    """Exporta las subcategorías de Notion a un CSV (elige ruta)."""
    try:
        if not CATEGORY_DATABASE_ID:
            messagebox.showerror("Configuración", "Falta NOTION_CATEGORY_DATABASE_ID en variables de entorno.")
            return

        rows = fetch_category_rows(CATEGORY_DATABASE_ID)
        if rows is None:
            messagebox.showerror("Error", "No se pudieron leer las subcategorías desde Notion.")
            if status_label:
                status_label.config(text="Error leyendo subcategorías desde Notion.")
            return
        ...
        if status_label:
            status_label.config(text=f"Subcategorías exportadas a {file_path}")
        messagebox.showinfo("Éxito", f"Subcategorías exportadas a {file_path}")
    except Exception as e:
        if status_label:
...
def process_file(bank_name: str, file_path: str, status_label=None) -> None:
    try:
        ...
        if existing_records is None:
            logging.error("No se pudieron leer los registros existentes de Notion.")
            if status_label:
                status_label.config(text="Error leyendo registros de Notion.")
            return
        ...
        if not data.empty:
            ...
            if status_label:
                status_label.config(
                    text=f"Proceso terminado. Insertados: {successful_inserts} | Repetidos: {repeated_records}"
                )
        else:
